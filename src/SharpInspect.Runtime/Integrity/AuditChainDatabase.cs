...
{
    /// <summary>
    /// The audit envelope generation of one stored schema generation. Schema 34
    /// and schema 35 keep the exact schema-33 audit envelope: the allowed metadata
    /// kinds grow,
    /// while every position column, envelope field, hash input, capacity reserve
    /// and membership rule stays identical. Every envelope-sensitive reader and
    /// writer therefore evaluates a schema-34 or schema-35 store at its 33 generation; only a
    /// caller that must prove the new generation itself reads the raw value.
    /// </summary>
    internal static int EnvelopeGeneration(long storedSchemaVersion) =>
        storedSchemaVersion is ProductionImageEvidenceStoreOptions.SchemaVersion or
            ProductionImageFinalizationStoreOptions.SchemaVersion or
            ProductionOutboxStoreOptions.SchemaVersion or
            ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or
            TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion
            ? RecipeLifecycleStoreOptions.SchemaVersion
            : checked((int)storedSchemaVersion);
    ...
    internal static string SchemaSqlFor(int version)
    {
        ...
        if (version == ProductionRecoveryStoreOptions.SchemaVersion)
            return SchemaSqlFor(PartIdentityStoreOptions.SchemaVersion).Replace(
        ...
        if (version == RecipeLifecycleStoreOptions.SchemaVersion)
            return RecipeLifecycleAuditSchema();
        if (version == ProductionImageEvidenceStoreOptions.SchemaVersion)
            return ImageEvidenceAuditSchema();
        if (version == ProductionImageFinalizationStoreOptions.SchemaVersion)
            return ImageFinalizationAuditSchema();
        if (version == ProductionOutboxStoreOptions.SchemaVersion)
            return ProductionOutboxAuditSchema();
        if (version == ProductionOutboxRecoveryOptions.SchemaVersion)
        {
            // Human operations retain the identity envelope. A handler block has its own
            // system metadata kind and never fabricates a physical send attempt.
            var sql = ProductionOutboxAuditSchema(withHandlerBlock: true);
            var applied = sql.Replace("PRAGMA user_version=36;", "PRAGMA user_version=37;", StringComparison.Ordinal);
            if (applied == sql) throw new InvalidOperationException("AuditSchemaDefinitionInvalid");
            return applied;
        }
        if (version == EvidenceReconciliationStoreOptions.SchemaVersion)
            return EvidenceReconciliationAuditSchema();
        if (version == TraceStorageRetentionOptions.SchemaVersion)
            return RetentionAuditSchema();
        if (version == DiagnosticSupportStoreOptions.SchemaVersion)
            return DiagnosticSupportAuditSchema();
        throw new ArgumentOutOfRangeException(nameof(version));
    }

    internal static bool IsCapacityReason(string reason) => reason is
        "RecipeSelectionTotalCapacityExceeded" or "RecipeSelectionRevisionCapacityExceeded" or
        "RecipeSelectionAuditPayloadCapacityExceeded" or "RecipeChangeEventCapacityExceeded" or
        ...
        "ProductionArmCapacityExceeded" or "ProductionArmAuditPayloadCapacityExceeded" or
        "RecipeLifecycleAuditCapacityExceeded" or "RecipeLifecycleAuditPayloadCapacityExceeded" or
        "RecipeLifecycleEntryCapacityExceeded" or "RecipeLifecyclePayloadCapacityExceeded" or "RecipeLifecycleTotalCapacityExceeded" or
        "ImageFinalizationAuditCapacityExceeded" or "ImageFinalizationAuditPayloadCapacityExceeded" or
        "ImageFinalizationEntryCapacityExceeded" or "ImageFinalizationPayloadCapacityExceeded" or
        "ImageFinalizationTotalCapacityExceeded" or "ImageFinalizationCapacityExceeded" or
        "ImageFinalizationReserveCapacityExceeded" or "ImageFinalizationReserveTotalCapacityExceeded" or
        "ProductionOutboxAuditCapacityExceeded" or "ProductionOutboxAuditPayloadCapacityExceeded" or
        "ProductionOutboxEntryCapacityExceeded" or "ProductionOutboxPayloadCapacityExceeded" or
        "ProductionOutboxTotalCapacityExceeded" or "ProductionOutboxCapacityExceeded" or
        "ProductionOutboxReserveCapacityExceeded" or "ProductionOutboxReserveTotalCapacityExceeded" or
        "ProductionOutboxRouteCapacityExceeded";
    ...
    private static (long Sequence, string PreviousHash, int SchemaVersion) NextSequence(
        ...
        long? recipeChangeReserveOverride = null, bool recipeLifecycleData = false,
        bool imageEvidenceData = false, bool imageFinalizationData = false,
        long? imageFinalizationReserveOverride = null,
        bool productionOutboxData = false, long? productionOutboxReserveOverride = null,
        long? evidenceReconciliationReserveOverride = null,
        long? retentionReserveOverride = null, long? checkpointReserveOverride = null,
        bool diagnosticSupportData = false, long? diagnosticSupportReserveOverride = null)
    {
        var schemaVersion = checked((int)EnvelopeGeneration(Scalar(db, "PRAGMA user_version;", deadline)));
        ...
        if (schemaVersion >= AlgorithmResultArchiveOptions.SchemaVersion)
        {
            ...
            if (schemaVersion is PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion)
                controlReserve = Math.Max(controlReserve, PlcCommunicationStoreOptions.ControlVerificationReserve);
            if (schemaVersion is ProductionInspectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion)
            ...
            if (schemaVersion == RecipeLifecycleStoreOptions.SchemaVersion)
                controlReserve = Math.Max(controlReserve, RecipeLifecycleStoreOptions.ControlVerificationReserve);
            // The image evidence activation entry reserves the same bounded control
            // budget as every other generation's activation entry.
            if (imageEvidenceData)
                controlReserve = Math.Max(controlReserve, ProductionImageEvidenceStoreOptions.ControlVerificationReserve);
            // The finalization ledger reserves the same bounded control budget as every other
            // generation's entries and its own outstanding completion reserve below.
            if (imageFinalizationData)
                controlReserve = Math.Max(controlReserve,
                    ProductionImageFinalizationStoreOptions.ControlVerificationReserve);
            var imageFinalizationConfigured = TableExists(db, "image_finalization_store_config", deadline) &&
                TableExists(db, "image_finalization_work", deadline);
            var imageFinalizationReserve = imageFinalizationConfigured
                ? imageFinalizationReserveOverride ??
                    SqliteCommandStore.ReadImageFinalizationAuditReserve(db, deadline)
                : 0;
            // The outbox keeps the complete remaining attempt/terminal footprint of every open
            // delivery reserved for every shared writer, so an admitted obligation can always
            // record every outcome it owes. A durable Admitted cycle that owns no Core row yet
            // reserves its complete uncreated batch on top: an override only replaces the
            // persisted part, so no writer can ever drop another pending cycle's liability.
            if (productionOutboxData)
                controlReserve = Math.Max(controlReserve,
                    ProductionOutboxStoreOptions.ControlVerificationReserve);
            var productionOutboxConfigured = TableExists(db, "production_outbox_store_config", deadline) &&
                TableExists(db, "production_outbox_deliveries", deadline);
            var productionOutboxReserve = !productionOutboxConfigured ? 0 :
                productionOutboxReserveOverride is { } overrideReserve
                    ? checked(overrideReserve +
                        SqliteCommandStore.ReadProductionOutboxPreCoreAuditReserve(db, deadline))
                    : SqliteCommandStore.ReadProductionOutboxAuditReserve(db, deadline);
            var reconciliationReserve = TableExists(db, "evidence_reconciliation_events", deadline)
                ? evidenceReconciliationReserveOverride ?? SqliteCommandStore.ReadEvidenceReconciliationAuditReserve(db, deadline)
                : 0;
            var retentionReserve = TableExists(db, "evidence_retention_events", deadline)
                ? retentionReserveOverride ?? SqliteCommandStore.ReadRetentionAuditReserve(db, deadline) : 0;
            var diagnosticReserve = TableExists(db, "diagnostic_operation_facts", deadline)
                ? diagnosticSupportReserveOverride ?? SqliteCommandStore.ReadDiagnosticSupportAuditReserve(db, deadline) : 0;
            var checkpointReserve = TableExists(db, "evidence_retention_events", deadline)
                ? checkpointReserveOverride ?? TraceCheckpointReserve(db, deadline) : 0;
            Require(checkpointReserve is 0 or 1, "TraceCheckpointReserveInvalid");
            var limit = checked(policy.MaximumVerificationEntries - controlReserve - retentionReserve - checkpointReserve -
                diagnosticReserve -
                ...
                partIdentityReserve - recipeChangeReserve - productionArmReserve - imageFinalizationReserve -
                productionOutboxReserve - reconciliationReserve);
            Require(sequence <= limit, archiveData
                ...
                : productionAdmissionData ? "ProductionAdmissionAuditCapacityExceeded"
                 ...
                 : recipeLifecycleData ? "RecipeLifecycleAuditCapacityExceeded"
                 : imageEvidenceData ? "ImageEvidenceAuditCapacityExceeded"
                 : imageFinalizationData ? "ImageFinalizationAuditCapacityExceeded"
                : productionOutboxData ? "ProductionOutboxAuditCapacityExceeded"
                : diagnosticSupportData ? "DiagnosticSupportAuditCapacityExceeded"
    ...
    internal static void AppendCommand(sqlite3 db, AuditIntegrityPolicy policy, IAuditSigningKey key,
        ...
        long? diagnosticSupportReserveOverride = null)
    {
        ...
        AppendEntry(db, policy, "CommandFact", position, payload, deadline,
            ...
            diagnosticSupportReserveOverride: diagnosticSupportReserveOverride);
    ...
    private static long AppendEntry(sqlite3 db, AuditIntegrityPolicy policy, string kind, long? position,
        ...
        bool recipeLifecycleData = false,
        bool imageEvidenceData = false,
        bool imageFinalizationData = false,
        long? imageFinalizationReserveOverride = null,
        bool productionOutboxData = false,
        long? productionOutboxReserveOverride = null,
        long? evidenceReconciliationReserveOverride = null,
        long? retentionReserveOverride = null, long? checkpointReserveOverride = null,
        bool diagnosticSupportData = false, long? diagnosticSupportReserveOverride = null)
    {
        var next = NextSequence(db, policy, deadline, archiveData: false,
            ...
            recipeLifecycleData: recipeLifecycleData,
            imageEvidenceData: imageEvidenceData,
            imageFinalizationData: imageFinalizationData,
            imageFinalizationReserveOverride: imageFinalizationReserveOverride,
            productionOutboxData: productionOutboxData,
            productionOutboxReserveOverride: productionOutboxReserveOverride,
            evidenceReconciliationReserveOverride: evidenceReconciliationReserveOverride,
            retentionReserveOverride: retentionReserveOverride, checkpointReserveOverride: checkpointReserveOverride,
            diagnosticSupportData: diagnosticSupportData,
            diagnosticSupportReserveOverride: diagnosticSupportReserveOverride);
    ...
    internal static long AppendIdentity(sqlite3 db, AuditIntegrityPolicy policy, IAuditSigningKey key,
        ...
        long? diagnosticSupportReserveOverride = null)
    {
        var storedSchemaVersion = checked((int)Scalar(db, "PRAGMA user_version;", deadline));
        var schemaVersion = EnvelopeGeneration(storedSchemaVersion);
        Require(schemaVersion is 3 or 4 or 5 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 or 26 or 27 or 28 or 29 or 30 or 31 or 32 or 33, "AuditSchemaInvalid");
        var next = NextSequence(db, policy, deadline, archiveData: false,
            cameraNetworkMode: cameraNetworkMode, manualInspectionReserveOverride: manualInspectionReserveOverride,
            productionAdmissionReserveOverride: productionAdmissionReserveOverride,
            stationQualificationReserveOverride: stationQualificationReserveOverride,
        ...
        var payload = fact.Encode(ordinal, storedSchemaVersion);
        IdentityAuditEvent.VerifyPayload(payload, ordinal, policy.StationId, storedSchemaVersion);
        var hash = schemaVersion >= PartIdentityStoreOptions.SchemaVersion
    ...
    {
        var schemaVersion = EnvelopeGeneration(Scalar(db, "PRAGMA user_version;", deadline));
    ...
    {
        Require(EnvelopeGeneration(Scalar(db, "PRAGMA user_version;", deadline)) is CalibrationSessionStoreOptions.SchemaVersion or
            CalibrationGovernanceStoreOptions.SchemaVersion or RecipeReleaseStoreOptions.SchemaVersion or PlcResultContractStoreOptions.SchemaVersion or RecipeActivationStoreOptions.SchemaVersion or PreviewSessionStoreOptions.SchemaVersion or CalibrationImportStoreOptions.SchemaVersion or ManualInspectionStoreOptions.SchemaVersion or ProductionAdmissionStoreOptions.SchemaVersion or StationQualificationStoreOptions.SchemaVersion or RecipeTransferStoreOptions.SchemaVersion or TraceStoragePolicyStoreOptions.SchemaVersion or QualificationCycleStoreOptions.SchemaVersion or PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion,
            "AuditSchemaInvalid");
    ...
    {
        var schemaVersion = EnvelopeGeneration(Scalar(db, "PRAGMA user_version;", deadline));
        Require(schemaVersion is ProductionInspectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion, "AuditSchemaInvalid");
        options.Validate();
    ...
    internal static AuditIntegrityReport Verify(sqlite3 db, AuditIntegrityPolicy policy, string trustedKeyId,
        ...
        StationQualificationStoreOptions? stationQualificationOptions = null,
         ...
         RecipeLifecycleStoreOptions? recipeLifecycleOptions = null,
         ProductionImageEvidenceStoreOptions? imageEvidenceOptions = null,
         ProductionImageFinalizationStoreOptions? imageFinalizationOptions = null,
         ProductionOutboxStoreOptions? productionOutboxOptions = null,
         ProductionStoreOptions? diagnosticSupportOptions = null)
    {
        var storedSchemaVersion = Scalar(db, "PRAGMA user_version;", deadline);
        Require(storedSchemaVersion is 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 or 26 or 27 or 28 or 29 or 30 or 31 or 32 or 33
            or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion
            or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion
            or DiagnosticSupportStoreOptions.SchemaVersion, "AuditSchemaInvalid");
        var schemaVersion = EnvelopeGeneration(storedSchemaVersion);
        ...
        var lifecycleVerification = BeginRecipeLifecycleVerification(db, schemaVersion, storedSchemaVersion,
            recipeLifecycleOptions, recipeDraftOptions, deadline);
        var imageEvidenceVerification = BeginImageEvidenceVerification(db, storedSchemaVersion, imageEvidenceOptions,
            deadline);
        var imageFinalizationVerification = BeginImageFinalizationVerification(db, storedSchemaVersion,
            imageFinalizationOptions, deadline);
        var productionOutboxVerification = BeginProductionOutboxVerification(db, storedSchemaVersion,
            productionOutboxOptions, deadline);
        var reconciliationVerification = BeginEvidenceReconciliationVerification(db, storedSchemaVersion, deadline);
        var retentionVerification = BeginRetentionVerification(db, storedSchemaVersion, deadline);
        var diagnosticSupportVerification = BeginDiagnosticSupportVerification(db, storedSchemaVersion,
            diagnosticSupportOptions, deadline);
        if (imageFinalizationOptions is not null)
        {
            Require(imageEvidenceOptions is not null, "ImageFinalizationImageEvidenceConfigurationRequired");
            Require(productionInspectionOptions is not null,
                "ImageFinalizationProductionConfigurationRequired");
            SqliteCommandStore.ValidateImageFinalizationHistory(db, imageFinalizationOptions,
                imageEvidenceOptions!, productionInspectionOptions!, deadline);
        }
        if (imageEvidenceOptions is not null)
        {
            Require(productionInspectionOptions is not null, "ImageEvidenceProductionConfigurationRequired");
            SqliteCommandStore.ValidateImageEvidenceHistory(db, imageEvidenceOptions, productionInspectionOptions!, deadline);
        }
        if (productionOutboxOptions is not null)
        {
            Require(productionInspectionOptions is not null,
                "ProductionOutboxProductionConfigurationRequired");
            SqliteCommandStore.ValidateProductionOutboxHistory(db, productionOutboxOptions,
        ...
        var schema29 = schemaVersion is PartIdentityStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion;
        ...
        if (schema28 || schemaVersion == ProductionRecoveryStoreOptions.SchemaVersion)
        ...
        var hasProductionArm = schema29 && productionArmOptions is not null &&
            TableExists(db, "production_arm_events", deadline);
        var hasLifecycle = lifecycleVerification is not null;
        ...
        var lifecycleActivations = hasLifecycle ? Scalar(db,
            "SELECT COUNT(*) FROM audit_entries WHERE Kind='RecipeLifecycleStoreActivated';", deadline) : 0;
        var hasImageEvidence = imageEvidenceVerification is not null;
        var imageEvidenceActivations = hasImageEvidence ? Scalar(db,
            "SELECT COUNT(*) FROM audit_entries WHERE Kind='ImageEvidenceStoreActivated';", deadline) : 0;
        var hasImageFinalization = imageFinalizationVerification is not null;
        var imageFinalizationActivations = hasImageFinalization ? Scalar(db,
            "SELECT COUNT(*) FROM audit_entries WHERE Kind='ImageFinalizationStoreActivated';", deadline) : 0;
        var hasProductionOutbox = productionOutboxVerification is not null;
        var productionOutboxActivations = hasProductionOutbox ? Scalar(db,
            "SELECT COUNT(*) FROM audit_entries WHERE Kind='ProductionOutboxStoreActivated';", deadline) : 0;
        var hasDiagnosticSupport = diagnosticSupportVerification is not null;
        var diagnosticSupportActivations = hasDiagnosticSupport ? Scalar(db,
            "SELECT COUNT(*) FROM audit_entries WHERE Kind='DiagnosticSupportActivated';", deadline) : 0;
        ...
        Require(!hasLifecycle || lifecycleActivations == 1, "RecipeLifecycleActivationMissing");
        Require(!hasImageEvidence || imageEvidenceActivations == 1, "ImageEvidenceActivationMissing");
        Require(!hasImageFinalization || imageFinalizationActivations == 1,
            "ImageFinalizationActivationMissing");
        Require(!hasProductionOutbox || productionOutboxActivations == 1,
            "ProductionOutboxActivationMissing");
        Require(!hasDiagnosticSupport || diagnosticSupportActivations == 1,
        ...
        Require(checked(maxFact + maxIdentity + maxAlarm + maxResult + maxDraft + maxCamera +
            maxRecovery + maxNetwork + maxImaging + maxCalibrationSession + maxCalibrationEvent + maxCalibrationManifest +
              ...
              (selectionVerification?.MetadataCount ?? 0) + (armVerification?.MetadataCount ?? 0) +
              (lifecycleVerification?.MetadataCount ?? 0) +
              (imageEvidenceVerification?.MetadataCount ?? 0) +
              (imageFinalizationVerification?.MetadataCount ?? 0) +
              (productionOutboxVerification?.MetadataCount ?? 0) +
               (reconciliationVerification?.MetadataCount ?? 0) + (retentionVerification?.MetadataCount ?? 0) +
               (diagnosticSupportVerification?.MetadataCount ?? 0)) == tail.Sequence - 1 && maxFact == Scalar(db,
        ...
        var prefixCheckpoint = startup && !fullArchiveVerification && !fullDraftVerification && !fullCameraVerification &&
            ...
            armVerification is null && lifecycleVerification is null ? checkpoint : ReadCheckpoint(db,
        ...
        foreach (var row in rows)
        {
            ...
            Require(previewKind || row.PreviewPosition is null, "AuditPreviewSessionPositionGap");
             ...
             Require(productionAdmissionKind || row.ProductionAdmissionPosition is null,
              ...
              Require(recipeTransferKind || row.RecipeTransferPosition is null,
               ...
               var lifecycleKind = row.Kind is "RecipeLifecycleStoreActivated" or "RecipeLifecycleEvent";
               var imageEvidenceKind = row.Kind is "ImageEvidenceStoreActivated";
               var imageFinalizationKind = SqliteCommandStore.IsImageFinalizationAuditKind(row.Kind) ||
                   row.Kind == SqliteCommandStore.ImageFinalizationActivationKind;
               var productionOutboxKind = SqliteCommandStore.IsProductionOutboxAuditKind(row.Kind) ||
                   row.Kind == SqliteCommandStore.ProductionOutboxActivationKind ||
                   row.Kind == SqliteCommandStore.ProductionOutboxRecoveryActivationKind;
               var reconciliationKind = row.Kind is SqliteCommandStore.EvidenceReconciliationActivationKind or
                   SqliteCommandStore.EvidenceReconciliationEventAuditKind;
                var retentionKind = row.Kind is SqliteCommandStore.RetentionActivationKind or SqliteCommandStore.RetentionEventAuditKind
                    or TraceCheckpointStarted or TraceCheckpointOutcome;
                var diagnosticSupportKind = row.Kind is SqliteCommandStore.DiagnosticSupportActivationKind or
                    SqliteCommandStore.DiagnosticOperationAuditKind;
                var encodedPayloadLimit = diagnosticSupportKind ?
                    DiagnosticSupportStoreOptions.MaximumPayloadBytesHardLimit * 2 + 1024 :
                    retentionKind ? (TraceStorageRetentionOptions.MaximumPayloadBytesHardLimit * 2 + 1024) * 2 :
                   reconciliationKind ?
                   (EvidenceReconciliationStoreOptions.MaximumPayloadBytesHardLimit * 2 + 1024) * 2 :
                   productionOutboxKind ? ProductionOutboxStoreOptions.MaximumAuditPayloadBytes * 2 :
                   imageFinalizationKind ? ProductionImageFinalizationStoreOptions.MaximumAuditPayloadBytes * 2 :
                   imageEvidenceKind ? ProductionImageEvidenceStoreOptions.MaximumAuditPayloadBytes * 2 :
                   lifecycleKind ? RecipeLifecycleStoreOptions.MaximumAuditPayloadBytes * 2 :
                   armKind ? ProductionArmStoreOptions.MaximumAuditPayloadBytes * 2 :
            ...
            var payload = Convert.FromBase64String(row.Payload);
              var payloadLimit = diagnosticSupportKind ? DiagnosticSupportStoreOptions.MaximumPayloadBytesHardLimit :
                  productionOutboxKind ? ProductionOutboxStoreOptions.MaximumAuditPayloadBytes :
                  lifecycleKind ? RecipeLifecycleStoreOptions.MaximumAuditPayloadBytes :
                  imageFinalizationKind ? ProductionImageFinalizationStoreOptions.MaximumAuditPayloadBytes :
                  imageEvidenceKind ? ProductionImageEvidenceStoreOptions.MaximumAuditPayloadBytes :
                  armKind ? ProductionArmStoreOptions.MaximumAuditPayloadBytes :
            ...
            else if (hasIdentity && row.Kind == "IdentityEvent")
            {
                ...
                IdentityAuditEvent.VerifyPayload(payload, identityOrdinal, policy.StationId, (int)storedSchemaVersion);
                if (retentionVerification is not null)
                    VerifyRetentionAuthorization(retentionVerification, row.Sequence, row.Hash!, payload);
                if (storedSchemaVersion is ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion or TraceStorageRetentionOptions.SchemaVersion or DiagnosticSupportStoreOptions.SchemaVersion)
                    SqliteCommandStore.VerifyProductionOutboxGovernanceAuditReference(db, payload, row.Sequence,
                        row.Hash, deadline);
            }
            ...
            else if (recipeSelectionKind && selectionVerification is not null)
            {
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
            ...
            else if (lifecycleKind && lifecycleVerification is not null)
            {
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    ...
                    row.ProductionAdmissionPosition is null && row.StationQualificationPosition is null && row.RecipeTransferPosition is null &&
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "RecipeLifecycleAuditPositionInvalid");
                VerifyRecipeLifecycleMetadata(db, lifecycleVerification, row.Kind, row.Sequence, row.Hash, payload, deadline);
            }
            else if (imageEvidenceKind && imageEvidenceVerification is not null)
            {
                // The schema-34 activation entry carries no typed position; a caller
                // without the option still fails closed on the shape, the payload bound
                // and the exact single activation count proved above.
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    row.CalibrationEventPosition is null && row.CalibrationManifestPosition is null && row.GovernancePosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "ImageEvidenceAuditPositionInvalid");
                VerifyImageEvidenceMetadata(db, imageEvidenceVerification, row.Kind, row.Sequence, row.Hash, payload,
                    deadline);
            }
            else if (imageFinalizationKind && imageFinalizationVerification is not null)
            {
                // The schema-35 activation entry and every finalization fact carry no typed
                // position; a caller without the option still fails closed on the shape, the
                // payload bound and the exact per-fact membership proved above.
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    row.CalibrationEventPosition is null && row.CalibrationManifestPosition is null && row.GovernancePosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "ImageFinalizationAuditPositionInvalid");
                VerifyImageFinalizationMetadata(db, imageFinalizationVerification, row.Kind, row.Sequence,
                    row.Hash, payload, deadline);
            }
            else if (retentionKind && retentionVerification is not null)
            {
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "RetentionAuditPositionInvalid");
                VerifyRetentionMetadata(retentionVerification, row.Kind, row.Sequence, row.Hash, payload);
            }
            else if (diagnosticSupportKind && diagnosticSupportVerification is not null)
            {
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "DiagnosticSupportAuditPositionInvalid");
                VerifyDiagnosticSupportMetadata(diagnosticSupportVerification, row.Kind, row.Sequence, row.Hash, payload);
            }
            else if (reconciliationKind && reconciliationVerification is not null)
            {
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "EvidenceReconciliationAuditPositionInvalid");
                VerifyEvidenceReconciliationMetadata(reconciliationVerification, row.Kind, row.Sequence, row.Hash, payload);
            }
            else if (productionOutboxKind && productionOutboxVerification is not null)
            {
                // The schema-36 activation entry and every outbox fact carry no typed position;
                // a caller without the option still fails closed on the shape, the payload
                // bound and the exact per-fact membership proved above.
                Require(row.FactPosition is null && row.IdentityPosition is null && row.AlarmPosition is null &&
                    row.ResultPosition is null && row.DraftPosition is null && row.CameraPosition is null &&
                    row.NetworkPosition is null && row.ImagingPosition is null && row.CalibrationSessionPosition is null &&
                    row.CalibrationEventPosition is null && row.CalibrationManifestPosition is null && row.GovernancePosition is null &&
                    ...
                    row.TraceStoragePolicyPosition is null && row.QualificationCyclePosition is null && row.PlcCommunicationPosition is null &&
                    row.ProductionInspectionPosition is null && row.PartIdentityPosition is null,
                    "ProductionOutboxAuditPositionInvalid");
                VerifyProductionOutboxMetadata(db, productionOutboxVerification, row.Kind, row.Sequence,
                    row.Hash, payload, deadline);
            }
        ...
        if (selectionVerification is not null)
        {
        ...
        if (lifecycleVerification is not null)
            Require(after == 0 && verifiedThrough == tail.Sequence, "RecipeLifecycleVerificationBudgetExceeded");
        ...
        if (hasActivation)
            SqliteCommandStore.VerifyRecipeActivationHistory(db, activationOptions!, releaseOptions!,
                contractOptions!, deadline, governanceOptions, recipeLifecycleOptions);
        if (recipeLifecycleOptions is not null)
            SqliteCommandStore.ReadRecipeLifecycleBusinessState(db, recipeLifecycleOptions, recipeDraftOptions!,
                releaseOptions, activationOptions, contractOptions, governanceOptions, recipeSelectionOptions, deadline);
        if (hasImport)
