...
public sealed class SqliteAuditIntegrityQuery : IAuditIntegrityQuery
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var result = await Task.Run(() =>
            {
                ...
                var storedSchemaVersion = AuditChainDatabase.Scalar(db, "PRAGMA user_version;", deadline);
                // Schema 34 keeps the exact schema-33 audit envelope; only the image
                // evidence generation itself is proved from the raw stored version.
                var schema = AuditChainDatabase.EnvelopeGeneration(storedSchemaVersion);
                ...
                if (_options.Outbox is null &&
                    storedSchemaVersion == ProductionOutboxStoreOptions.SchemaVersion)
                    throw new InvalidOperationException("ProductionOutboxConfigurationRequired");
                if (_options.Outbox is not null &&
                    storedSchemaVersion < ProductionOutboxStoreOptions.SchemaVersion)
                    throw new InvalidOperationException("ProductionOutboxGovernedMigrationRequired");
                if (storedSchemaVersion >= ProductionOutboxStoreOptions.SchemaVersion &&
                    (_options.ProductionInspections is null || _options.TraceStoragePolicies is null ||
                        _options.LocalIdentity is null))
                    throw new InvalidOperationException(
                        "ProductionOutboxRequiresInspectionsTracePolicyIdentityAndAudit");
                if (_options.ImageEvidence is not null &&
                ...
                if (storedSchemaVersion >= ProductionImageEvidenceStoreOptions.SchemaVersion &&
                    _options.ImageEvidence is not null &&
                    (_options.ProductionInspections is null || _options.RecipeDrafts is null ||
                        _options.RecipeReleases is null || _options.RecipeLifecycle is null ||
                        _options.ProductionArming is null || _options.LocalIdentity is null))
                    throw new InvalidOperationException(
                        "ImageEvidenceRequiresInspectionsDraftsReleasesLifecycleArmingIdentityAndAudit");
                if (_options.ImageFinalization is null &&
                    storedSchemaVersion == ProductionImageFinalizationStoreOptions.SchemaVersion)
                ...
                if (storedSchemaVersion >= ProductionImageFinalizationStoreOptions.SchemaVersion &&
                    _options.ImageFinalization is not null &&
                    (_options.ImageEvidence is null || _options.ProductionInspections is null ||
                        _options.RecipeDrafts is null || _options.RecipeReleases is null ||
                        _options.RecipeLifecycle is null || _options.ProductionArming is null ||
                        _options.LocalIdentity is null))
                    throw new InvalidOperationException(
                        "ImageFinalizationRequiresImageEvidenceAndProductionStack");
                ...
                if (_options.RecipeLifecycle is null && schema == RecipeLifecycleStoreOptions.SchemaVersion &&
                    storedSchemaVersion < ProductionOutboxStoreOptions.SchemaVersion)
                    throw new InvalidOperationException("RecipeLifecycleConfigurationRequired");
                if (_options.RecipeLifecycle is not null && schema < RecipeLifecycleStoreOptions.SchemaVersion)
                    throw new InvalidOperationException("RecipeLifecycleGovernedMigrationRequired");
                if (schema == RecipeLifecycleStoreOptions.SchemaVersion &&
                    storedSchemaVersion < ProductionOutboxStoreOptions.SchemaVersion &&
                    (_options.RecipeDrafts is null || _options.LocalIdentity is null))
                    throw new InvalidOperationException("RecipeLifecycleRequiresDraftsIdentityAndAudit");
                ...
                if ((schema == ManualInspectionStoreOptions.SchemaVersion ||
                     ((schema is ProductionAdmissionStoreOptions.SchemaVersion or StationQualificationStoreOptions.SchemaVersion or RecipeTransferStoreOptions.SchemaVersion or TraceStoragePolicyStoreOptions.SchemaVersion or QualificationCycleStoreOptions.SchemaVersion or PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion) && _options.ManualInspections is not null)) &&
                ...
                AuditChainDatabase.Require(schema is 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 or 26 or 27 or 28 or 29 or 30 or 31 or 32 or 33,
                ...
                var cameraSchema = (schema is CameraSetupStoreOptions.SchemaVersion or CameraRecoveryStoreOptions.SchemaVersion or
                    ...
                    RecipeActivationStoreOptions.SchemaVersion or PreviewSessionStoreOptions.SchemaVersion or
                     CalibrationImportStoreOptions.SchemaVersion or ManualInspectionStoreOptions.SchemaVersion or
                     ProductionAdmissionStoreOptions.SchemaVersion or StationQualificationStoreOptions.SchemaVersion or RecipeTransferStoreOptions.SchemaVersion or TraceStoragePolicyStoreOptions.SchemaVersion or QualificationCycleStoreOptions.SchemaVersion or PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion) && _options.CameraSetup is not null;
                ...
                var governanceSchema = (schema is CalibrationGovernanceStoreOptions.SchemaVersion or
                    ...
                    RecipeActivationStoreOptions.SchemaVersion or PreviewSessionStoreOptions.SchemaVersion or
                     CalibrationImportStoreOptions.SchemaVersion or ManualInspectionStoreOptions.SchemaVersion or
                     ProductionAdmissionStoreOptions.SchemaVersion or StationQualificationStoreOptions.SchemaVersion or RecipeTransferStoreOptions.SchemaVersion or TraceStoragePolicyStoreOptions.SchemaVersion or QualificationCycleStoreOptions.SchemaVersion or PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or ProductionRecoveryStoreOptions.SchemaVersion or RecipeSelectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion or ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion) && _options.CalibrationGovernance is not null;
                ...
                var fullVerification = archiveVerification || draftVerification || cameraSchema || recoverySchema || networkVerification ||
                    ...
                    _options.PartIdentities is not null || _options.Outbox is not null;
                ...
                recipeLifecycleOptions: _options.RecipeLifecycle, imageEvidenceOptions: _options.ImageEvidence, imageFinalizationOptions: _options.ImageFinalization,
            ...
            StationQualificationReadGuard.RequireVerified(db, report, deadline, _options);
                ...
                if (schema is ProductionArmStoreOptions.SchemaVersion or RecipeLifecycleStoreOptions.SchemaVersion or ProductionImageEvidenceStoreOptions.SchemaVersion or ProductionImageFinalizationStoreOptions.SchemaVersion or ProductionOutboxStoreOptions.SchemaVersion or ProductionOutboxRecoveryOptions.SchemaVersion or EvidenceReconciliationStoreOptions.SchemaVersion)
                    AuditChainDatabase.RequireFullProductionArmVerification(db, report, deadline);
                if (schema == RecipeLifecycleStoreOptions.SchemaVersion && _options.RecipeLifecycle is not null)
                    AuditChainDatabase.RequireFullRecipeLifecycleVerification(db, report, deadline,
    ...
    {
        if (exception is InvalidOperationException { Message: var reconciliationReason } &&
            reconciliationReason.Length <= 128 &&
            reconciliationReason.StartsWith("EvidenceReconciliation", StringComparison.Ordinal) &&
            reconciliationReason.All(c => c is >= 'A' and <= 'Z' or >= 'a' and <= 'z' or >= '0' and <= '9'))
        ...
        if (exception is InvalidOperationException { Message: var armReason } &&
            ...
            return armReason;
        if (exception is InvalidOperationException { Message: var lifecycleReason } &&
            lifecycleReason.StartsWith("RecipeLifecycle", StringComparison.Ordinal))
            return lifecycleReason;
        if (exception is InvalidOperationException { Message: var imageEvidenceReason } &&
            imageEvidenceReason.StartsWith("ImageEvidence", StringComparison.Ordinal))
            return imageEvidenceReason;
        if (exception is InvalidOperationException { Message: var productionImageReason } &&
            productionImageReason.StartsWith("ProductionImage", StringComparison.Ordinal))
            return productionImageReason;
        if (exception is InvalidOperationException { Message: var imageFinalizationReason } &&
            imageFinalizationReason.StartsWith("ImageFinalization", StringComparison.Ordinal))
            return imageFinalizationReason;
        if (exception is InvalidOperationException { Message: var outboxReason } &&
            outboxReason.StartsWith("ProductionOutbox", StringComparison.Ordinal))
            return outboxReason;
