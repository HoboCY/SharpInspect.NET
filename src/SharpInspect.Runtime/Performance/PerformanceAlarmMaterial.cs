...
internal static class PerformanceAlarmMaterial
{
    internal const string Code = "PerformanceBudgetViolation";
    internal const string Source = "Runtime.Performance";

    internal static bool IsNonBlockingInstance(AlarmPolicy policy, AlarmInstanceSnapshot instance) =>
        instance.Code == Code && instance.Source == Source &&
        instance.PolicyId == policy.Id && instance.PolicyVersion == policy.Version &&
        instance.PolicyContentHash == policy.ContentHash &&
        policy.TryGetRule(Code, out var rule) && rule is not null && rule.Source == Source &&
