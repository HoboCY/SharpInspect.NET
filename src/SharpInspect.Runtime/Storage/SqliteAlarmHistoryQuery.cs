...
public sealed class SqliteAlarmHistoryQuery : IAlarmHistoryQuery
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var schema20 = schema == CalibrationImportStoreOptions.SchemaVersion;
            ...
            var schema25 = schema is TraceStoragePolicyStoreOptions.SchemaVersion or QualificationCycleStoreOptions.SchemaVersion or PlcCommunicationStoreOptions.SchemaVersion or ProductionInspectionStoreOptions.SchemaVersion or PartIdentityStoreOptions.SchemaVersion;
            ...
            StationQualificationReadGuard.RequireConfiguration(schema, _options);
                AuditChainDatabase.Require(schema is 7 or 8 or 9 or 10 or 11 or 12 or 13 or 14 or 15 or 16 or 17 or 18 or 19 or 20 or 21 or 22 or 23 or 24 or 25 or 26 or 27 or 28 or 29,
                    schema < PlcCommunicationStoreOptions.SchemaVersion ? "GovernedAlarmMigrationRequired" : "StoreSchemaTooNew");
            ...
            if (_options.ManualInspections is null && schema21)
            ...
            if (_options.RecipeReleases is null && (schema18 || schema19 || schema20))
                throw new InvalidOperationException("RecipeReleaseConfigurationRequired");
            ...
            if (_options.PlcResultContracts is null && (schema18 || schema19 || schema20))
                throw new InvalidOperationException("PlcResultContractConfigurationRequired");
            ...
            if (_options.PreviewSessions is null && (schema19 || schema20))
            ...
            if (_options.CalibrationImports is null && schema20)
            ...
            if (_options.ProductionAdmission is null && schema22)
                throw new InvalidOperationException("ProductionAdmissionConfigurationRequired");
            if (schema20 && (_options.PreviewSessions is null || _options.CalibrationGovernance is null ||
            ...
            if ((schema18 || schema19 || schema20 || schema21) && _options.RecipeDrafts is null)
                throw new InvalidOperationException("RecipeDraftConfigurationRequired");
            ...
            if (_options.CalibrationSessions is null &&
                (schema is CalibrationSessionStoreOptions.SchemaVersion or CalibrationGovernanceStoreOptions.SchemaVersion or
                    CalibrationImportStoreOptions.SchemaVersion))
            ...
            if (_options.CameraRecovery is null &&
                (schema is CameraRecoveryStoreOptions.SchemaVersion or CalibrationGovernanceStoreOptions.SchemaVersion or
                    CalibrationImportStoreOptions.SchemaVersion))
            ...
            var networkStore = modernOptional ? _options.CameraNetwork is not null :
            ...
            var imagingStore = modernOptional ? _options.ImagingSetup is not null :
                schema is ImagingSetupStoreOptions.SchemaVersion or CalibrationSessionStoreOptions.SchemaVersion or
                CalibrationGovernanceStoreOptions.SchemaVersion;
            var calibrationStore = modernOptional ? _options.CalibrationSessions is not null :
                schema is CalibrationSessionStoreOptions.SchemaVersion or CalibrationGovernanceStoreOptions.SchemaVersion;
            ...
            var releaseStore = modernOptional ? _options.RecipeReleases is not null :
                schema16 || schema17 || schema18 || schema19 || schema20;
            var contractStore = modernOptional ? _options.PlcResultContracts is not null :
                schema17 || schema18 || schema19 || schema20;
            var activationStore = modernOptional ? _options.RecipeActivations is not null :
                schema18 || schema19 || schema20;
            var previewStore = modernOptional ? _options.PreviewSessions is not null : schema19 || schema20;
            var importStore = modernOptional ? _options.CalibrationImports is not null : schema20;
                   ...
                   releaseOptions: releaseStore ? _options.RecipeReleases : null,
                    ...
                    previewOptions: previewStore ? _options.PreviewSessions : null,
                     importOptions: importStore ? _options.CalibrationImports : null,
                      manualOptions: schema21 || schema22 || schema23 || schema24 || schema25 ? _options.ManualInspections : null,
                      productionAdmissionOptions: schema22 || schema23 || schema24 || schema25 ? _options.ProductionAdmission : null,
            ...
            if (releaseStore)
             ...
             if (previewStore)
              ...
              if (importStore)
               ...
               if ((schema21 || schema22 || schema23 || schema24 || schema25) && _options.ManualInspections is not null)
                  AuditChainDatabase.RequireFullManualInspectionVerification(database, verification, deadline,
                      _options.ManualInspections);
               if ((schema22 || schema23 || schema24 || schema25) && _options.ProductionAdmission is not null)
...
internal static class AlarmStorageCodec
{
    ...
    private static void ValidateRecord(AlarmHistoryRecord record, long expectedPosition, AlarmPolicy? policy)
    {
        ...
        if (record.Transition == AlarmTransitionKind.PolicyActivated)
        {
            ...
            throw new InvalidOperationException("AlarmPolicyActivationUnexpected");
        // A healthy source observed before its first fault has no alarm instance.
        // Preserve that existing transition contract without inventing an incident.
        var sourceObservation = record.Transition == AlarmTransitionKind.Observed &&
            record.ReasonCode == "AlarmObserved" && record.Code is not null && record.Source is not null &&
            record.InstanceId is null && record.ActorPrincipalId is null && record.SessionId is null &&
            record.CommandCorrelationId is null && record.PlcProjection is null;
        if (record.Transition is not (AlarmTransitionKind.PolicyActivated or AlarmTransitionKind.BoundaryRejected or AlarmTransitionKind.ProjectionChanged) &&
            record.Instance is null && !sourceObservation)
...
internal sealed partial class SqliteCommandStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return await Task.Run(() =>
        {
            ...
            var importStore = modernOptional ? _options.CalibrationImports is not null : schema20;
             ...
             alarmStore || archiveStore || draftStore || cameraStore || recoveryStore || networkStore || imagingStore || calibrationStore || governanceStore || releaseStore || contractStore || activationStore || previewStore || importStore ? new AuditVerificationRequest(0, _policy.MaximumVerificationEntries) :
                           new AuditVerificationRequest(), !alarmStore && !archiveStore && !draftStore && !cameraStore && !recoveryStore && !networkStore && !imagingStore && !calibrationStore && !governanceStore && !releaseStore && !contractStore && !activationStore && !previewStore && !importStore, deadline,
        ...
        StoreDeadline deadline)
    {
        ...
        try
        {
            ...
            var governanceStore = _options.CalibrationGovernance is not null;
            ...
            var verification = AuditChainDatabase.Verify(database, _policy!, _signingKey!.KeyId,
                _signingKey.PublicKeyBase64,
                alarmStore || archiveStore || draftStore || cameraStore || recoveryStore || networkStore || imagingStore || calibrationStore || governanceStore || releaseStore || contractStore || activationStore || previewStore || importStore || manualStore ? new AuditVerificationRequest(0, _policy!.MaximumVerificationEntries) :
                    new AuditVerificationRequest(), !alarmStore && !archiveStore && !draftStore && !cameraStore && !recoveryStore && !networkStore && !imagingStore && !calibrationStore && !governanceStore && !releaseStore && !contractStore && !activationStore && !previewStore && !importStore && !manualStore, deadline,
                validateAnchorReceipt: false,
