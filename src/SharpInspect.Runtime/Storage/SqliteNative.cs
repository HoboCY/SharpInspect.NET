...
internal static class SqliteNative
{
        ...
        ProductionStoreOptions options, long schemaVersion = 0)
    {
        ...
        if (options.PreviewSessions is not null)
            limit = Math.Max(limit, PreviewSessionStoreOptions.SqliteValueLimitBytes);
        ...
        if (options.StationQualifications is not null)
            limit = Math.Max(limit, StationQualificationStoreOptions.SqliteValueLimitBytes);
        ...
        if (options.PartIdentities is not null)
            limit = Math.Max(limit, PartIdentityStoreOptions.SqliteValueLimitBytes);
        if (options.RecipeSelections is not null)
            limit = Math.Max(limit, RecipeSelectionStoreOptions.SqliteValueLimitBytes);
        if (options.ProductionArming is not null)
            limit = Math.Max(limit, ProductionArmStoreOptions.SqliteValueLimitBytes);
        if (options.RecipeLifecycle is not null)
            limit = Math.Max(limit, RecipeLifecycleStoreOptions.SqliteValueLimitBytes);
        ...
        var schemaLimit = schemaVersion switch
        {
            ...
            19 => Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(CalibrationSessionStoreOptions.SqliteValueLimitBytes,
                    CalibrationGovernanceStoreOptions.SqliteValueLimitBytes),
                RecipeReleaseStoreOptions.SqliteValueLimitBytes),
                PlcResultContractStoreOptions.SqliteValueLimitBytes),
                RecipeActivationStoreOptions.SqliteValueLimitBytes),
                PreviewSessionStoreOptions.SqliteValueLimitBytes),
            20 => Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(CalibrationSessionStoreOptions.SqliteValueLimitBytes,
                    CalibrationGovernanceStoreOptions.SqliteValueLimitBytes),
                RecipeReleaseStoreOptions.SqliteValueLimitBytes),
                PlcResultContractStoreOptions.SqliteValueLimitBytes),
                RecipeActivationStoreOptions.SqliteValueLimitBytes),
            ...
            21 => Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(Math.Max(CalibrationSessionStoreOptions.SqliteValueLimitBytes,
                    CalibrationGovernanceStoreOptions.SqliteValueLimitBytes),
                RecipeReleaseStoreOptions.SqliteValueLimitBytes),
                PlcResultContractStoreOptions.SqliteValueLimitBytes),
                RecipeActivationStoreOptions.SqliteValueLimitBytes),
            ...
            25 or 26 or 27 or 28 or 29 or 30 or 31 or 32 or 33 or 34 or 35 or 36 or 37 or 38 or 39 => new[]
            {
                CalibrationSessionStoreOptions.SqliteValueLimitBytes,
                CalibrationGovernanceStoreOptions.SqliteValueLimitBytes,
                RecipeReleaseStoreOptions.SqliteValueLimitBytes,
                ...
                schemaVersion >= RecipeSelectionStoreOptions.SchemaVersion ? RecipeSelectionStoreOptions.SqliteValueLimitBytes : 0,
                schemaVersion >= ProductionArmStoreOptions.SchemaVersion ? ProductionArmStoreOptions.SqliteValueLimitBytes : 0,
                schemaVersion >= RecipeLifecycleStoreOptions.SchemaVersion ? RecipeLifecycleStoreOptions.SqliteValueLimitBytes : 0
                , schemaVersion >= ProductionImageFinalizationStoreOptions.SchemaVersion
                    ? ProductionImageFinalizationStoreOptions.SqliteValueLimitBytes : 0
                , schemaVersion >= ProductionOutboxStoreOptions.SchemaVersion
                    ? ProductionOutboxStoreOptions.SqliteValueLimitBytes : 0
            }.Max(),
            _ => 65536
        };
