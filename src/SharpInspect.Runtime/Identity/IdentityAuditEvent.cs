...
internal sealed record IdentityAuditEvent(Guid EventId, IdentityEventKind Kind, DateTimeOffset OccurredAtUtc,
    ...
    int HashTargetCost = 0, int RecordCost = 0)
{
    ...
    internal static long VerifyPayload(byte[] payload, long ordinal, string stationId)
    {
        ...
        try
        {
            AuditChainDatabase.Require(ReadInteger() == AuditCanonical.CanonicalizationVersion &&
                ReadValue() == "IdentityEvent" && ReadInteger() == 18, "AuditIdentityPayloadInvalid");
            var fields = Enumerable.Range(0, 18).Select(_ => ReadValue()).ToArray();
