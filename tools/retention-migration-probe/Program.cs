// Compiled independently against preserved schema-38 packages and the schema-39 candidate.
...
#endif
{
    ...
    await using (var provider = services.BuildServiceProvider())
    {
        ...
        {
            // Schema 38 already starts reconciliation asynchronously. Its first
            // committed event can invalidate the earlier Verified observation.
            // Await that public startup boundary and its verified audit tail
            // before the one-shot seed command; do not retry failed writes.
            ...
            do
            {
                ...
                if (startup.LatestStartup?.Completed == true &&
                    ...
                    verified.VerifiedThroughSequence >= startup.ThroughAuditSequence) break;
        ...
        if (snapshot.Store.State == HealthState.Healthy)
        {
            ...
            var retention = provider.GetRequiredService<IEvidenceRetentionService>();
            var retained = await retention.ReadAsync(new());
            var capacity = await provider.GetRequiredService<ITraceStorageCapacityQuery>().ReadAsync();
            if (!retained.Available || retained.Records.Count != 0 || retained.ExecutionPolicy is null ||
                retained.ExecutionPolicy.DeletableClasses.Count != 0 || capacity.Available || capacity.MaximumWalBytes is not null)
                throw new InvalidOperationException("ProbeRetentionPublicBoundaryInvalid");
            operations = new { verificationId = "V155_N03", serviceResolved = true, queryAvailable = true,
                retentionAvailable = retained.Available, retentionRecords = retained.Records.Count,
                deletionClasses = retained.ExecutionPolicy.DeletableClasses.Count, capacityAvailable = capacity.Available,
                capacityReason = capacity.ReasonCode, maximumWalBytes = capacity.MaximumWalBytes,
...
static ProductionStoreOptions Options(string directory)
{
    ...
    const string station = "V155.PackageProbe";
    var name = "SharpInspect.Test.V155.Probe." + Convert.ToHexString(SHA256.HashData(
    ...
    var authoring = new AuthorizationPolicy("V155.Probe.Authoring", "1",
    ...
    {
        Blocklist = PasswordBlocklist.Create("V155.Probe.Blocklist", "1", new[] { "known-compromised-value" })
    ...
    return new ProductionStoreOptions(Path.Combine(directory, "store.sqlite"))
    {
        ...
        RecipeDrafts = new RecipeDraftStoreOptions(new AlgorithmExecutionPolicy("V155.Probe.Execution", "1",
        ...
        RecipeReleases = new RecipeReleaseStoreOptions(new RecipeGovernancePolicy(
            "V155.Probe.ReleaseGovernance", "1", RecipeGovernanceMode.MakerCheckerRelease)),
        ProductionInspections = new ProductionInspectionStoreOptions(),
        TraceStoragePolicies = new TraceStoragePolicyStoreOptions
        {
            DeploymentScope = new TraceStorageDeploymentScope("V155.Probe.Deployment", "1",
        ...
        AlarmPolicy = new AlarmPolicy("V155.Probe.Images.Alarm", "1", new[]
        {
...
#endif
#if CURRENT_MIGRATION
        StorageRetention = new(new TraceRetentionExecutionPolicy("V155.Probe.Retention", "1", "isolated package verification",
            "No deletion class enabled in the migration consumer.", Array.Empty<TraceRetentionClass>(),
            new(TimeSpan.FromMinutes(5), TimeSpan.FromSeconds(2), 1024 * 1024, 4), 2),
