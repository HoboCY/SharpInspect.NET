...
$consumerRoot = Join-Path ([IO.Path]::GetFullPath($Run)) 'part-identity-consumer'
...
var partFormat = new PartIdentityFormat("Consumer.PartFormat", "1", 1, 32, " ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-");
var partBinding = new PartIdentityProviderBinding("Consumer.PartBinding", "1", "Part.Code",
    PartIdentityProviderSourceKind.Staged, "Consumer.Scanner", "1", new string('B', 64), partFormat,
    TimeSpan.FromSeconds(2), TimeSpan.FromMilliseconds(200), 1);
var partRequirement = new PartIdentityRequirement(PartIdentityRequirementMode.Required, partBinding.LogicalRole,
    partFormat.ToContractReference());
var partProvider = new StagedPartIdentityProvider(partBinding);
var partCycle = new PartIdentityCycleBinding(Guid.NewGuid(), profile.EndpointBindingHash, 1, 71, 1);
var partStage = partProvider.Stage(new(Guid.NewGuid(), partCycle, " A-001 "));
if (!partStage.Accepted) throw new InvalidOperationException(partStage.ReasonCode);
var partCapabilities = await partProvider.GetCapabilitiesAsync();
var partRequest = new PartIdentityLatchRequest(partBinding, partCycle, 1, partCapabilities.SourceEpoch,
    partCapabilities.SourceGeneration);
var partFirst = await partProvider.TryLatchAsync(partRequest);
var partSecond = await partProvider.TryLatchAsync(partRequest);
if (partFirst.Status != PartIdentityObservationStatus.Present || partFirst.Value != " A-001 " ||
    partSecond.Status != PartIdentityObservationStatus.Missing || partSecond.Value != null ||
    partFirst.StageToken != partStage.StageToken || partFirst.Cycle.ContentHash != partCycle.ContentHash)
    throw new InvalidOperationException("PublicPartIdentityAtomicLatchFailed");
partProvider.RestartSource();
var partOldSource = await partProvider.TryLatchAsync(partRequest);
if (partOldSource.Status != PartIdentityObservationStatus.Error)
    throw new InvalidOperationException("OldPartIdentitySourceWasReplayed");
if (typeof(PartIdentityLatchRequest).GetProperty("InspectionId") is not null ||
    typeof(PartIdentityEvidence).GetConstructors(BindingFlags.Public | BindingFlags.Instance).Length != 0)
    throw new InvalidOperationException("PublicPartIdentityEvidenceAuthorityChanged");
if (!partFormat.TryValidate(" A-001 ", out _) || partFormat.TryValidate("bad", out _))
    throw new InvalidOperationException("PublicPartIdentityRawFormatChanged");

Console.WriteLine(JsonSerializer.Serialize(new
{
    CaseId = "V143_N01",
    PartIdentityValue = partFirst.Value,
    PartIdentityBindingHash = partBinding.ContentHash,
    PartIdentityRequirementHash = partRequirement.ContentHash,
    PartIdentityObservationHash = partFirst.ContentHash,
    PartIdentitySecondStatus = partSecond.Status.ToString(),
    PartIdentityOldSourceStatus = partOldSource.Status.ToString(),
    Result = "Pass",
...
function Invoke-PartIdentityConsumerDotnet([string]$LogName, [string[]]$Arguments) {
...
Invoke-PartIdentityConsumerDotnet 'part-identity-consumer-restore.log' @(
    'restore', $projectPath, '--configfile', (Join-Path $consumerRoot 'NuGet.Config'),
    '--packages', $packages)
Invoke-PartIdentityConsumerDotnet 'part-identity-consumer-build.log' @(
...
Invoke-PartIdentityConsumerDotnet 'part-identity-consumer-run.log' @($consumerDll)
$consumerOutput = Get-Content -LiteralPath (Join-Path ([IO.Path]::GetFullPath($Run)) 'part-identity-consumer-run.log') -Raw
$evidence = $consumerOutput | ConvertFrom-Json
if ($evidence.CaseId -cne 'V143_N01' -or $evidence.Result -cne 'Pass' -or
    $evidence.PartIdentityValue -cne ' A-001 ' -or
    $evidence.PartIdentitySecondStatus -cne 'Missing' -or
    $evidence.PartIdentityOldSourceStatus -cne 'Error' -or
...
$evidence | ConvertTo-Json -Depth 6 |
    Set-Content -LiteralPath (Join-Path ([IO.Path]::GetFullPath($Run)) 'part-identity-consumer-evidence.json') -Encoding utf8
Write-Output 'V143_N01 isolated NuGet part identity provider, single consumption, source restart and authority boundaries PASS; production execution NOT_RUN.'
