param(
    ...
    [Parameter(Mandatory)][ValidateSet(38)][int]$SourceSchema,
    [int[]]$Phases = @(1..14),
...
$migrationRoot = Join-Path $migrationRun ('retention-' + $SourceSchema)
if (Test-Path -LiteralPath $migrationRoot) { throw 'Use a fresh retention migration consumer directory.' }
...
$migrationTargetSchema = 39
$migrationVerificationPrefix = 'V155'
...
function Build-MigrationConsumer([string]$Name, [string]$Feed, [bool]$Current) {
    ...
    $probeSource = 'retention-migration-probe/Program.cs'
...
foreach ($migrationPhase in $Phases) {
    ...
    if ($current.operations.verificationId -cne 'V155_N03' -or
        $current.operations.serviceResolved -cne $true -or $current.operations.queryAvailable -cne $true -or
        $current.operations.startupCompleted -cne $true -or $current.operations.PendingQuarantines -ne 0 -or
        $current.operations.IntegrityFaultRecorded -cne $false -or $current.operations.records -lt 2 -or
        $current.operations.throughAuditSequence -le 0) {
        throw 'Schema 39 public retention and reconciliation query or current startup proof failed.'
    }
    if ($current.operations.retentionAvailable -ne $true -or $current.operations.retentionRecords -ne 0 -or
        $current.operations.deletionClasses -ne 0 -or $current.operations.capacityAvailable -ne $false -or
        $null -ne $current.operations.maximumWalBytes) { throw 'Public retention/capacity boundary failed.' }
...
$migrationEvidence | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $migrationRun ('retention-migration-' + $SourceSchema + '-consumer.json')) -Encoding utf8
