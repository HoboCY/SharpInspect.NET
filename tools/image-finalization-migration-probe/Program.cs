// Compiled independently against the preserved schema-34 packages and against
...
#endif
{
    ...
    var page = await new SqliteProductionImageEvidenceQuery(options).QueryAsync(new ProductionImageEvidenceFilter());
    ...
    evidence = new { phase = 0, Ready = snapshot.Ready, state = snapshot.Store.State.ToString(),
        ...
        auditLedgerChanged = auditLedgerBefore != auditLedgerAfter, imageFinalization = imageWork };
...
{
    const string station = "V151.PackageProbe";
    var name = "SharpInspect.Test.V151.Probe." + Convert.ToHexString(SHA256.HashData(
    ...
    var authoring = new AuthorizationPolicy("V151.Probe.Authoring", "1",
    ...
    {
        Blocklist = PasswordBlocklist.Create("V151.Probe.Blocklist", "1", new[] { "known-compromised-value" })
    ...
    return new ProductionStoreOptions(Path.Combine(directory, "store.sqlite"))
    {
        ...
        RecipeDrafts = new RecipeDraftStoreOptions(new AlgorithmExecutionPolicy("V151.Probe.Execution", "1",
        ...
        RecipeReleases = new RecipeReleaseStoreOptions(new RecipeGovernancePolicy(
            "V151.Probe.ReleaseGovernance", "1", RecipeGovernanceMode.MakerCheckerRelease)),
