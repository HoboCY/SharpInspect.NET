...
$qualificationCopy = Join-Path $qualificationRun 'station-qualification-consumer'
$qualificationEvidence = Join-Path $qualificationRun 'station-qualification-demo'
...
Invoke-QualificationDotnet 'station-qualification-consumer-restore.log' @(
    'restore', $qualificationProject, '-p:UseLocalPackages=true', '--configfile', $qualificationConfig,
    '--packages', (Join-Path $qualificationRun 'station-qualification-consumer-cache'))
Invoke-QualificationDotnet 'station-qualification-consumer-build.log' @(
...
Invoke-QualificationDotnet 'station-qualification-consumer-run.log' @(
    $qualificationDll, '--station-qualification-check', $qualificationEvidence)
Invoke-QualificationDotnet 'station-qualification-consumer-cold-query.log' @(
    $qualificationDll, '--station-qualification-query', $qualificationEvidence)
$qualificationDetail = Get-Content -LiteralPath (Join-Path $qualificationEvidence 'station-qualification-evidence.json') -Raw | ConvertFrom-Json
$qualificationCold = Get-Content -LiteralPath (Join-Path $qualificationEvidence 'station-qualification-restart.json') -Raw | ConvertFrom-Json
if ($qualificationDetail.Result -cne 'Pass' -or $qualificationDetail.CaseId -cne 'V137_N01' -or
    $qualificationDetail.SchemaVersion -ne 23 -or
    ...
    $qualificationCold.Result -cne 'Pass' -or $qualificationCold.CaseId -cne 'V137_N02' -or
...
Write-Output 'V137 independent NuGet consumer PASS; actual physical qualification NOT_RUN.'
