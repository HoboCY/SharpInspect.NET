param(
    ...
    [Parameter(Mandatory)][ValidateSet(35,36,37)][int]$SourceSchema,
    [int[]]$Phases = @(1..14),
...
$migrationRoot = Join-Path $migrationRun ('reconcile-' + $SourceSchema)
...
$migrationTargetSchema = 38
$migrationVerificationPrefix = 'V154'
...
function Build-MigrationConsumer([string]$Name, [string]$Feed, [bool]$Current) {
    ...
    $probeSource = 'evidence-reconciliation-migration-probe/Program.cs'
...
"@
    ...
    & dotnet restore $project --configfile $config --source $Feed --source $DependencyPackageFeed --packages (Join-Path $consumer 'cache') *> (Join-Path $consumer 'restore.log')
...
foreach ($migrationPhase in $Phases) {
    ...
    if ($current.operations.verificationId -cne 'V154_N03' -or
        ...
        throw 'Schema 38 public reconciliation query or current startup proof failed.'
    }
...
$migrationEvidence | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $migrationRun ('evidence-reconciliation-migration-' + $SourceSchema + '-consumer.json')) -Encoding utf8
