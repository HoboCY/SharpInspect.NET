param(
    ...
    [Parameter(Mandatory)][string]$Schema34PackageFeed,
    [int[]]$Phases = @(1..14)
)
...
$migrationRoot = Join-Path $migrationRun 'png-migration-consumer'
if (Test-Path -LiteralPath $migrationRoot) { throw 'Use a fresh image evidence migration consumer directory.' }
[void][IO.Directory]::CreateDirectory($migrationRoot)
$migrationCurrentFeed = [IO.Path]::GetFullPath($PackageFeed)
$migrationOldFeed = [IO.Path]::GetFullPath($Schema34PackageFeed)
if ($migrationCurrentFeed -eq $migrationOldFeed) { throw 'The old writer must come from independently preserved schema-34 packages.' }
foreach ($migrationPhase in $Phases) {
    if ($migrationPhase -lt 1 -or $migrationPhase -gt 14) { throw 'Unsupported public migration phase.' }
}
# The preserved schema-34 writer has no schema-35 PNG finalization feature: its
# startup preflight throws StoreSchemaTooNew, and the writer's own failure
# filter surfaces TraceStoreUnavailable, exactly as the preserved T49 run
# recorded for the 32 -> 33 generation jump.
$migrationOldRefusalReason = 'TraceStoreUnavailable'
$migrationSourceSchema = 34
$migrationTargetSchema = 35
...
function Build-MigrationConsumer([string]$Name, [string]$Feed, [bool]$Current) {
    ...
    Copy-Item -LiteralPath (Join-Path $PSScriptRoot 'image-finalization-migration-probe/Program.cs') -Destination (Join-Path $consumer 'Program.cs')
...
foreach ($migrationPhase in $Phases) {
    ...
    if ($seed.schema -ne $migrationSourceSchema -or $seed.state -ne 'Healthy' -or $seed.audit -ne 'Persisted' -or
        $seed.auditIntegrity -ne 'Verified' -or $seed.auditLedgerBefore -cne 'absent' -or $seed.Ready -or
        $seed.assemblyHash -cne $migrationOld.runtimeHash) {
        throw 'Preserved old writer did not create a real writable schema-34 database.'
    }
    ...
    if (-not $resumed.Completed -or $resumed.schema -ne $migrationTargetSchema -or
        ...
        throw 'Cold resume did not prove the schema-35 generation, verified backup and signed history.'
    }
    ...
    if ($current.state -ne 'Healthy' -or $current.audit -ne 'Persisted' -or $current.schema -ne $migrationTargetSchema -or
        $current.Ready -or $current.auditIntegrity -ne 'Verified' -or
    ...
    if (-not $current.imageFinalization.available -or $current.imageFinalization.items -ne 0 -or
        $current.imageFinalization.stageRoot -notlike '*image-stage' -or
        $current.imageFinalization.stageContentHash -notmatch '^[0-9A-Fa-f]{64}$') {
        throw ('The schema-35 store did not expose the empty image finalization work page: ' + $current.imageFinalization.reason)
    }
    $migrationCases += [ordered]@{ id='V151_N02'; phase=$migrationPhase; result='Pass'; interruptedBy='Process.Kill(entireProcessTree:true)';
        ...
        imageFinalizationItems=$current.imageFinalization.items; imageFinalizationReason=$current.imageFinalization.reason }
    Write-Output "V151 phase $migrationPhase crash/resume, schema-34 writer refusal and empty schema-35 image finalization work query PASS"
}
$migrationEvidence = [ordered]@{ id='V151_N01'; result='Pass';
    scope='isolated package consumers, verified SQLite backup, whole-process termination at the explicitly listed public phases; schema-34 to schema-35; completed phase checks reopen idempotency; no physical station, staged image bytes, PNG finalization or production qualification';
...
$migrationEvidence | ConvertTo-Json -Depth 10 | Set-Content -LiteralPath (Join-Path $migrationRun 'image-finalization-migration-consumer.json') -Encoding utf8
